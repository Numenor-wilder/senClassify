...
class ImplicitExtractor(object):
    ...
    def __init__(self, hashtags_dict: str):
        hashtags_dict_path = os.path.join(GlobalPath.DICT_PATH, hashtags_dict)
        self.hashtags = self.__load_hashtags(hashtags_dict_path)  # hashtags


    def __load_hashtags(self, hashtags_dict_path: str) -> list:
        '''
        载入文件，返回hashtags列表
        机器规则
        ...
        剩余标签：
        :param hashtags_dict_path: hashtags路径默认位于./fenci/dict/hot_hashtags.txt
        :return: list of hashtags
        '''
        res = []
        try:
            file = open(hashtags_dict_path, 'r', encoding='utf-8')
            ...
            logging.error("路径" + hashtags_dict_path + "不存在 或 未指定hashtags路径，因此hashtags=[] 关于hashtags处理无效")
        return res


    def features_judgement(self, sentence: str) -> list:
        '''
        隐式特征类型判断器,判断是否存在隐式特征结果，传回分析的结果内容
        :param sentence:
        :return: list of ImplicitType
        '''
        res = []
        # 工整句式
        if len(self.__neat_sentence_extractor(sentence)) != 0:
            res.append(self.ImplicitType.NEAT_SENTENCE)
        # 热点hashtags
        if len(self.__hot_hashtags_extractor(sentence, self.hashtags)) != 0:
            res.append(self.ImplicitType.HOT_HASHTAG)
        return res


    def __hot_hashtags_extractor(self, content: str, hot_hashtags: list) -> list:
        '''
    ...
    def __neat_sentence_extractor(self, content: str) -> str:
        '''
        ...
        if content.count('：') + content.count(':') > 0:
            rs = re.split('[：:]', content)
            for r in rs:
                result = self.__neat_sentence_extractor(r)
                if result:
                    return result
            return ""
        else:
...
#         level=logging.INFO)
#     imp = ImplicitExtractor(hashtags_file_path=r'E:\workspace\Gitrepo\senClassify\resources\text\fenci\dict\hot_hashtags.txt')
